                ...
                Some(path) => {
                    ...
                    quote!({
                        ...
                        _serde::__private::Option::map(
                            attempt!(_serde::de::SeqAccess::next_element::<#wrapper_ty>(&mut __seq)),
                            |__wrap| __wrap.value)
                    })
                }
            };
    ...
    let write_values = fields.iter().map(|field| {
        ...
        if field.attrs.skip_deserializing() {
            ...
            let write = match field.attrs.deserialize_with() {
                None => {
                    quote! {
                        if let _serde::__private::None = attempt!(_serde::de::SeqAccess::next_element_seed(&mut __seq,
                ...
                Some(path) => {
                    ...
                    quote!({
                        #wrapper
                        match attempt!(_serde::de::SeqAccess::next_element::<#wrapper_ty>(&mut __seq)) {
...
fn deserialize_externally_tagged_enum(
    ...
    let match_variant = if all_skipped {
        ...
        quote! {
            match attempt!(_serde::de::EnumAccess::variant(__data)) {
                #(#variant_arms)*
            }
        }
    };
...
fn deserialize_internally_tagged_enum(
    ...
    quote_block! {
        ...
        let __tagged = attempt!(_serde::Deserializer::deserialize_any(
            __deserializer,
...
fn deserialize_adjacently_tagged_enum(
    ...
    let next_key = quote! {
        attempt!(_serde::de::MapAccess::next_key_seed(&mut __map, #tag_or_content))
    };
            ...
            while let _serde::__private::Some(__k) = #next_key {
                match __k {
                    _serde::__private::de::TagContentOtherField::Other => {
                        attempt!(_serde::de::MapAccess::next_value::<_serde::de::IgnoredAny>(&mut __map));
                        continue;
                    },
    ...
    let finish_content_then_tag = if variant_arms.is_empty() {
        quote! {
            match attempt!(_serde::de::MapAccess::next_value::<__Field>(&mut __map)) {}
        }
    } else {
        quote! {
            let __ret = attempt!(match attempt!(_serde::de::MapAccess::next_value(&mut __map)) {
        ...
        impl #de_impl_generics _serde::de::Visitor<#delife> for __Visitor #de_ty_generics #where_clause {
                ...
                __A: _serde::de::MapAccess<#delife>,
            {
                ...
                match #next_relevant_key {
                    ...
                    _serde::__private::Some(_serde::__private::de::TagOrContentField::Tag) => {
                        // Parse the tag.
                        let __field = attempt!(_serde::de::MapAccess::next_value(&mut __map));
                        ...
                        match #next_relevant_key {
                            ...
                            _serde::__private::Some(_serde::__private::de::TagOrContentField::Content) => {
                                let __ret = attempt!(_serde::de::MapAccess::next_value_seed(&mut __map,
                                    __Seed {
                                        field: __field,
                    ...
                    _serde::__private::Some(_serde::__private::de::TagOrContentField::Content) => {
                        ...
                        let __content = attempt!(_serde::de::MapAccess::next_value::<_serde::__private::de::Content>(&mut __map));
                        // Visit the second key.
                        match #next_relevant_key {
                ...
                __A: _serde::de::SeqAccess<#delife>,
            {
                ...
                match attempt!(_serde::de::SeqAccess::next_element(&mut __seq)) {
                    _serde::__private::Some(__field) => {
                        // Visit the second element - the content.
                        match attempt!(_serde::de::SeqAccess::next_element_seed(
                            &mut __seq,
                            __Seed {
                                field: __field,
                                marker: _serde::__private::PhantomData,
...
fn deserialize_untagged_enum(
    ...
    quote_block! {
        let __content = attempt!(<_serde::__private::de::Content as _serde::Deserialize>::deserialize(__deserializer));
...
fn deserialize_externally_tagged_variant(
    ...
    match variant.style {
        Style::Unit => {
            let this = &params.this;
            quote_block! {
                attempt!(_serde::de::VariantAccess::unit_variant(__variant));
                _serde::__private::Ok(#this::#variant_ident)
            }
        }
...
fn deserialize_internally_tagged_variant(
    ...
    match effective_style(variant) {
        Style::Unit => {
            ...
            quote_block! {
                attempt!(_serde::Deserializer::deserialize_any(#deserializer, _serde::__private::de::InternallyTaggedUnitVisitor::new(#type_name, #variant_name)));
...
fn deserialize_map(
    ...
    let ignored_arm = if cattrs.has_flatten() {
        Some(quote! {
            __Field::__other(__name) => {
                __collect.push(_serde::__private::Some((
                    __name,
                    attempt!(_serde::de::MapAccess::next_value(&mut __map)))));
            }
        })
        ...
        Some(quote! {
            _ => { let _ = attempt!(_serde::de::MapAccess::next_value::<_serde::de::IgnoredAny>(&mut __map)); }
        })
    };
    ...
    let match_keys = if cattrs.deny_unknown_fields() && all_skipped {
        quote! {
            // FIXME: Once we drop support for Rust 1.15:
            // let _serde::__private::None::<__Field> = attempt!(_serde::de::MapAccess::next_key(&mut __map));
            _serde::__private::Option::map(
                attempt!(_serde::de::MapAccess::next_key::<__Field>(&mut __map)),
                |__impossible| match __impossible {});
        }
    } else {
        quote! {
            while let _serde::__private::Some(__key) = attempt!(_serde::de::MapAccess::next_key::<__Field>(&mut __map)) {
                match __key {
                    #(#value_arms)*
                    #ignored_arm
                }
    ...
    let extract_collected = fields_names
        ...
        .map(|(field, name)| {
            ...
            quote! {
                let #name: #field_ty = attempt!(#func(
    ...
    let value_arms_from = fields_names
        ...
        .map(|(field, name)| {
            ...
            let visit = match field.attrs.deserialize_with() {
                None => {
                    quote! {
                        attempt!(_serde::de::MapAccess::next_value_seed(&mut __map, _serde::__private::de::InPlaceSeed(&mut self.place.#member)))
                    }
                }
                Some(path) => {
                    let (wrapper, wrapper_ty) = wrap_deserialize_field_with(params, field.ty, path);
        ...
        impl #de_impl_generics _serde::Deserialize<#delife> for __DeserializeWith #de_ty_generics #where_clause {
                ...
                __D: _serde::Deserializer<#delife>,
            {
                _serde::__private::Ok(__DeserializeWith {
                    value: attempt!(#deserialize_with(__deserializer)),
