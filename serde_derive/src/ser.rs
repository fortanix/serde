...
fn serialize_tuple_struct(
    ...
    quote_block! {
        let #let_mut __serde_state = attempt!(_serde::Serializer::serialize_tuple_struct(__serializer, #type_name, #len));
        #(#serialize_stmts)*
...
fn serialize_struct_tag_field(cattrs: &attr::Container, struct_trait: &StructTrait) -> TokenStream {
    match cattrs.tag() {
        attr::TagType::Internal { tag } => {
            ...
            quote! {
                attempt!(#func(&mut __serde_state, #tag, #type_name));
            }
        }
        _ => quote! {},
    }
...
fn serialize_struct_as_struct(
    ...
    quote_block! {
        let #let_mut __serde_state = attempt!(_serde::Serializer::serialize_struct(__serializer, #type_name, #len));
        #tag_field
        #(#serialize_fields)*
...
fn serialize_struct_as_map(
    ...
    quote_block! {
        let #let_mut __serde_state = attempt!(_serde::Serializer::serialize_map(__serializer, #len));
        #tag_field
        #(#serialize_fields)*
...
fn serialize_internally_tagged_variant(
    ...
    match effective_style(variant) {
        Style::Unit => {
            quote_block! {
                let mut __struct = attempt!(_serde::Serializer::serialize_struct(
                    __serializer, #type_name, 1));
                attempt!(_serde::ser::SerializeStruct::serialize_field(
...
fn serialize_tuple_variant(
    ...
    match context {
        TupleVariant::ExternallyTagged {
            ...
            quote_block! {
                let #let_mut __serde_state = attempt!(_serde::Serializer::serialize_tuple_variant(
                    __serializer,
                    #type_name,
                    #variant_index,
                    #variant_name,
        ...
        TupleVariant::Untagged => {
            quote_block! {
                let #let_mut __serde_state = attempt!(_serde::Serializer::serialize_tuple(
                    __serializer,
                    #len));
                #(#serialize_stmts)*
...
fn serialize_struct_variant<'a>(
    ...
    match context {
        StructVariant::ExternallyTagged {
            ...
            quote_block! {
                let #let_mut __serde_state = attempt!(_serde::Serializer::serialize_struct_variant(
                    __serializer,
                    #name,
                    #variant_index,
                    #variant_name,
        ...
        StructVariant::InternallyTagged { tag, variant_name } => {
            quote_block! {
                let mut __serde_state = attempt!(_serde::Serializer::serialize_struct(
                    __serializer,
                    #name,
                    #len + 1,
                ));
        ...
        StructVariant::Untagged => {
            quote_block! {
                let #let_mut __serde_state = attempt!(_serde::Serializer::serialize_struct(
                    __serializer,
                    #name,
                    #len,
                ));
            ...
            quote_block! {
                ...
                impl #wrapper_impl_generics _serde::Serialize for __EnumFlatten #wrapper_ty_generics #where_clause {
                        ...
                        __S: _serde::Serializer,
                    {
                        ...
                        let #let_mut __serde_state = attempt!(_serde::Serializer::serialize_map(
                            __serializer,
        ...
        StructVariant::InternallyTagged { tag, variant_name } => {
            quote_block! {
                let #let_mut __serde_state = attempt!(_serde::Serializer::serialize_map(
                    __serializer,
                    _serde::__private::None));
                attempt!(_serde::ser::SerializeMap::serialize_entry(
                    &mut __serde_state,
                    #tag,
                    #variant_name,
                ));
...
fn serialize_tuple_struct_visitor(
    ...
    fields
        ...
        .map(|(i, field)| {
            ...
            let ser = quote! {
                attempt!(#func(&mut __serde_state, #field_expr));
            };
    ...
    fields
        ...
        .map(|field| {
            ...
            let ser = if field.attrs.flatten() {
                let func = quote_spanned!(span=> _serde::Serialize::serialize);
                quote! {
                    attempt!(#func(&#field_expr, _serde::__private::ser::FlatMapSerializer(&mut __serde_state)));
                }
            } else {
                let func = struct_trait.serialize_field(span);
                quote! {
                    attempt!(#func(&mut __serde_state, #key_expr, #field_expr));
                }
            };
                    ...
                    if let Some(skip_func) = struct_trait.skip_field(span) {
                        quote! {
                            if !#skip {
                                ...
                                attempt!(#skip_func(&mut __serde_state, #key_expr));
                            }
                        }
                    } else {
                        quote! {
...
//
//     let mut __serde_state = attempt!(serializer.serialize_struct("S", 0));
//     _serde::ser::SerializeStruct::end(__serde_state)
//
